...
public class SlidingPaneLayout extends ViewGroup {
    ...
    private float lastInterceptMoveX;
    
    private int currentViewPagerIndex = 0;
    
    public void setCurrentViewPagerIndex(int item) {
    	currentViewPagerIndex = item;
    }
    
    @Override
    public boolean onInterceptTouchEvent(MotionEvent ev) {
    	Log.d(tag, "start onInterceptTouchEvent action======="+ev.getAction());
//    	if(isOpen()) return true;
    	boolean interceptReturn = false;
...
//        return interceptForDrag || interceptTap;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
    	Log.d(tag, "start onTouchEvent action======="+ev.getAction());
        if (!mCanSlide) {
            return super.onTouchEvent(ev);
        }
        
        Log.d(tag, "start onTouchEvent actionindex======="+ev.getActionIndex());
