...
public abstract class Request<T> implements Comparable<Request<T>> {
	...
	/**
	 * When a request can be retrieved(取回) from cache but must be refreshed from the
	 * network, the cache entry will be stored here so that in the event of(万一) a
	 * "Not Modified" response, we can be sure it hasn't been evicted(依法收回) from
	 * cache.
	 */
	...
	/**
	 * Returns the annotated(注释的) cache entry, or null if there isn't one.
	 */
	 ...
	 * encoded string.
	 */
	protected byte[] encodeParameters(Map<String, String> params,
	 ...
	 * 
	 * @return This Request object to allow for chaining(链起来).
	 */
