	...
	public HttpResponse performRequest(Request<?> request,
		...
		for (String headerName : map.keySet()) {
			if ("Cookie".equals(headerName)) {
				String[] split = map.get("Cookie").split("\n");
				for (String s : split) {
					connection.addRequestProperty(headerName, s);
				}
			} else {
		...
		Logcat.d(TAG, "performRequest: HurlStack : " + url +" : "+ Thread.currentThread().getName() + ":" + Thread.currentThread().getId());
		
		for(Map.Entry<String, String> s :request.getHeaders().entrySet()){
			Logcat.d(TAG, s.getKey() + "  " + s.getValue());
		}
